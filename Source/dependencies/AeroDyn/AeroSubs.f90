...
DO NFOILID = 1, p%AirFoil%NUMFOIL
 ...
 ! These are retained for future USE and backwards compatibility only
   ...
   READ(NUNIT,'( A )',IOSTAT=IOS) LINE
   IF ( IOS < 0 ) THEN
      CALL PremEOF ( Trim(p%AirFoil%FOILNM(NFOILID)) , 'Stall angle (deg)', .TRUE., ErrMessLcl )
      close(NUNIT)
      CALL SetErrStat( ErrID_Fatal, ErrMessLcl, ErrStat, ErrMess, 'READFL')
      RETURN
   END IF
   IF (p%DSTALL) READ(LINE,*,ERR=205)  (O%Beddoes%StallAngle( NFOILID, K ), K = 1, p%AirFoil%NTables(NFOILID))   
   !!! Umass WInDS   
   ! .......................................................   
 ...
 ! Convert angles to radians
   IF (p%DSTALL) THEN
      ...
      !..............................
      !!! Umass WInDS
      O%Beddoes%StallAngle( NFOILID, : ) = O%Beddoes%StallAngle( NFOILID, : )*D2R   
      !!! Umass WInDS    
      !................................
   ENDIF
